...
pygame.mixer.music.load('grasswalk.mp3')  # Reemplaza con la ruta de tu archivo de música
...
def game_loop():
    ...
    while not termino:
        ...
        # Manejo de entradas del joystick
        if pygame.joystick.get_count() > 0:
...
def main():
    ...
    areajuego.fill(fondo)
    if score > 0:
        message_display("Los Zombies te ganaron "+str(score),300,450)
    else:
        message_display("Tu calificacion sobrevivio!!!",300,450)
    pygame.display.update()
    time.sleep(4)
    pygame.quit()
    quit()
